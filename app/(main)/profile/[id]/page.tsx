      ...
      const projectsWithContributors = await Promise.all(
        (contributionsData || []).map(async (contribution) => {
          ...
          return {
            ...
            contributors:
              projectContributors?.map((c) => c.profiles).filter(Boolean) || [],
          };
        })
      );
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-lg text-onsurface-secondary">
                {profile.title} {profile.location && `in ${profile.location}`}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                {profile.projects.map((project) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="text-lg font-medium text-onsurface-primary mb-1">
                          {project.title}
                        </h3>
                        <p className="text-onsurface-secondary text-sm leading-relaxed">
                          {project.description ||
                            profile.bio ||
                            "No description available"}
                        </p>
                      </div>
                      <span className="text-sm text-onsurface-secondary ml-4 whitespace-nowrap">
                    ...
                    <div className="bg-surface-container-muted rounded-xl border border-border p-4">
                      <p className="text-sm font-medium text-onsurface-primary mb-2">
                        {project.role}
                      </p>
                      <p className="text-sm text-onsurface-secondary mb-3 leading-relaxed">
                              ...
                              .map((contributor) => (
                                <div key={contributor.id} className="relative">
                                  {contributor.avatar_url ? (
                                    ...
                                    <div className="w-8 h-8 rounded-full bg-surface-container border-2 border-background flex items-center justify-center">
                                      <User
                                        size={16}
