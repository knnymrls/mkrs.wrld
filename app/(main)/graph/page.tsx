...
import GraphModeSelector, {
  GraphMode,
} from "@/app/components/features/graph/GraphModeSelector";
...
export default function GraphPage() {
  ...
  useEffect(() => {
    if (graphRef.current && filteredData.nodes.length > 0) {
      ...
      if (charge) charge.strength(-200); // Reduced repulsion for closer nodes
      if (link) link.distance(60); // Reduced distance between nodes
  ...
  useEffect(() => {
    ...
    console.log("Filtered results:", {
      ...
      filteredProfiles: filteredNodes.filter((n) => n.type === "profile")
        .length,
      filteredPosts: filteredNodes.filter((n) => n.type === "post").length,
      filteredProjects: filteredNodes.filter((n) => n.type === "project")
        .length,
    });
  ...
  useEffect(() => {
    ...
    const profileCount = filteredData.nodes.filter(
      (n) => n.type === "profile"
    ).length;
    const postCount = filteredData.nodes.filter(
      (n) => n.type === "post"
    ).length;
    const projectCount = filteredData.nodes.filter(
      (n) => n.type === "project"
    ).length;

    const totalConnections = Object.values(nodeConnectionCounts).reduce(
      (sum, count) => sum + count,
      0
    );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const projectMentionLinks: Link[] = (postProjects || []).map(
        (pp: any) => ({
  ...
  const handleSearch = (query: string) => {
    ...
    const profileMatch = filteredData.nodes.find(
      (node) =>
        node.type === "profile" && node.label.toLowerCase().includes(lowerQuery)
    );
    ...
    if (profileMatch) {
      ...
      if (graphRef.current && (profileMatch as any).x !== undefined) {
        graphRef.current.centerAt(
          (profileMatch as any).x,
          (profileMatch as any).y,
          1000
        );
                ...
                .filter((node) => {
                  ...
                  if (searchQuery.trim()) {
                    ...
                    return (
                      ...
                      (node.type === "profile" &&
                        (node as ProfileNode).title
                          ?.toLowerCase()
                          .includes(query)) ||
                      (node.type === "project" &&
                        (node as ProjectNode).description
                          ?.toLowerCase()
                          .includes(query))
                    );
                  }
                          ...
                          <div className="p-4 flex flex-col gap-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <h3 className="font-medium text-onsurface-primary truncate">
                                  {profile.label}
                                </h3>
                                {profile.title && (
                            ...
                            {profile.skills && profile.skills.length > 0 && (
                              <div className="flex flex-wrap gap-1.5">
                                {profile.skills.slice(0, 4).map((skill, i) => (
                                  <span
                                    key={i}
                                    className="px-2 py-0.5 bg-surface-container-muted text-onsurface-primary text-xs rounded-lg"
                                  >
                                    {skill}
                                  </span>
                                ))}
                      ...
                      >
                        ...
                        >
                          <div className="p-4 flex flex-col gap-3">
                            ...
                            {project.description && (
                              <p className="text-sm text-onsurface-primary line-clamp-2">
                                {project.description}
                              </p>
                            )}
                          </div>
            ...
            <div className="relative w-full h-full overflow-hidden">
              ...
              {(hoveredNode || clickedNode) &&
                (() => {
                  ...
                  const nodeData = filteredData.nodes.find(
                    (n) => n.id === nodeId
                  );
                  ...
                  return (
                    <div
                      className={`absolute top-4 right-4 z-20 bg-surface-container border border-border rounded-xl shadow-lg p-5 w-72 ${
                        clickedNode
                          ? "pointer-events-auto"
                          : "pointer-events-none"
                      }`}
                    >
                      ...
                      <div className="flex items-start gap-3 mb-4">
                        ...
                        (nodeData as ProfileNode).avatar_url ? (
                          <Image
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-foreground text-base mb-0.5 truncate">
                            {nodeData.label.length > 24
                              ? nodeData.label.slice(0, 24) + "..."
                              : nodeData.label}
                          </h3>
                          <p className="text-xs text-onsurface-secondary capitalize">
                            {nodeData.type}
                      ...
                      <div className="space-y-3 text-sm">
                        {nodeData.type === "profile" && (
                          <>
                            ...
                            {(nodeData as ProfileNode).skills &&
                              (nodeData as ProfileNode).skills!.length > 0 && (
                                <div className="flex flex-wrap gap-1.5">
                                  {(nodeData as ProfileNode)
                                    .skills!.slice(0, 5)
                                    .map((skill, i) => (
                                      <span
                                        key={i}
                                        className="px-2.5 py-1 bg-surface-container-muted text-onsurface-primary text-xs rounded-lg font-medium"
                                      >
                                        {skill}
                                      </span>
                                    ))}
                                  {(nodeData as ProfileNode).skills!.length >
                                    5 && (
                                ...
                                <div
                                  ...
                                  style={{
                                    backgroundColor:
                                      (nodeData as ProjectNode).status ===
                                      "active"
                                        ? canvasColors.statusActive
                                        : (nodeData as ProjectNode).status ===
                                          "paused"
                                        ? canvasColors.statusPaused
                        ...
                        <div className="pt-3 mt-3 border-t border-border/30 flex items-center justify-between">
                          <span className="text-sm text-onsurface-secondary">
                            Connections
                          </span>
                            ...
                            <button
                              onClick={() => {
                                if (nodeData.type === "profile") {
                                  ...
                                  const projectId = nodeData.id.replace(
                                    "project-",
                                    ""
                                  );
                                  router.push(`/projects/${projectId}`);
              ...
              {currentMode === "skill-radar" ? (
                ...
                <ForceGraph2D
                  ...
                  nodeCanvasObject={(node, ctx, globalScale) => {
                    const isRelated =
                      !hoveredNode || relatedNodes.has(node.id as string);
                    ...
                    const connections = nodeId
                      ? nodeConnectionCounts[nodeId] || 0
                      : 0;
                    const importance = nodeId ? nodeImportance[nodeId] || 0 : 0;
                    ...
                    if (
                      node.type === "profile" &&
                      (node as ProfileNode).avatar_url
                    ) {
                    ...
                    const shouldShowLabel =
                      (node.type === "profile" || node.type === "project") &&
                      isFinite(node.x!) &&
                      isFinite(node.y!) &&
                    ...
                    if (shouldShowLabel) {
                      const label =
                        node.label && node.label.length > 30
                      ...
                      if (importance <= 0.6 && !isHovered) {
                        ...
                        labelOpacity = Math.max(
                          0,
                          Math.min((globalScale - 1.05) * 3, 1)
                        );
                      }
                      ...
                      if (!hoveredNode && importance <= 0.6) {
                        ...
                        ctx.globalAlpha = isRelated
                          ? labelOpacity
                          : labelOpacity * 0.08; // Softer fade
                      }
                  ...
                  linkCanvasObject={(link, ctx) => {
                    ...
                    const isRelated =
                      !hoveredNode ||
                      (relatedNodes.has(source.id) &&
                        relatedNodes.has(target.id));
                    ...
                    ctx.lineWidth =
                      isRelated && hoveredNode
                        ? 1.2
                        : 0.5 + linkImportance * 0.3; // Thinner, softer
                  ...
                  nodePointerAreaPaint={(node, color, ctx) => {
                    ...
                    const connections =
                      nodeConnectionCounts[node.id as string] || 0;
